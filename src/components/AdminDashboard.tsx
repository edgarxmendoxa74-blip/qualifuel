...
const AdminDashboard: React.FC = () => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(() => localStorage.getItem('beracah_admin_auth') === 'true');
                  ...
                  {(menuItems || []).slice(0, 5).map(item => (
                    <div key={item.id} className="px-12 py-8 flex items-center justify-between hover:bg-white/5 transition-colors">
                      <div className="flex items-center space-x-6">
                        <div className="h-14 w-14 bg-white/5 rounded-2xl flex items-center justify-center text-3xl border border-white/10">{categories.find(cat => cat.id === item.category)?.icon || '\ud83e\udd57'}</div>
                        <div>
