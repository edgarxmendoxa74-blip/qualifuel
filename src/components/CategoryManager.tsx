      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-16 gap-8">
        ...
        {currentView === 'list' ? (
          ...
          <div className="flex space-x-4 w-full md:w-auto">
            ...
            <button onClick={handleSaveCategory} className="flex-1 md:flex-none bg-quali-gradient text-white px-10 py-5 rounded-2xl md:rounded-[1.5rem] hover:shadow-[0_0_30px_rgba(154,202,60,0.2)] transition-all font-black uppercase tracking-widest text-[10px] border border-white/10 shadow-lg">
              <Save className="h-4 w-4 mr-2 inline" /> Deploy
            </button>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {categories.length === 0 ? (
            <div className="col-span-full py-32 text-center bg-white/[0.02] rounded-[3.5rem] border-2 border-dashed border-white/5">
              ...
              <p className="text-gray-500 font-black uppercase tracking-[0.3em] text-xs mb-10">No Provisioning Sectors Identified</p>
            ...
            categories.map((category) => (
              <div key={category.id} className="bg-white/5 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/10 group hover:border-quali-primary/40 transition-all shadow-2xl relative overflow-hidden">
                <div className="absolute -top-10 -right-10 text-9xl opacity-5 grayscale group-hover:grayscale-0 transition-all duration-700 -rotate-12 translate-x-4 translate-y-4">
                   {category.icon}
                </div>
                
                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-8">
                    <div className="text-5xl bg-black/40 w-20 h-20 flex items-center justify-center rounded-3xl border border-white/10 shadow-inner transform group-hover:scale-110 transition-transform duration-500">
                      {category.icon}
                    </div>
                  ...
                  <div>
                    <div className="flex items-center space-x-2 mb-2">
                       <div className={`h-2 w-2 rounded-full ${category.active ? 'bg-quali-primary shadow-[0_0_10px_rgba(154,202,60,0.8)]' : 'bg-red-500'}`} />
                       <h3 className="text-2xl font-black text-white italic tracking-tighter uppercase leading-none">{category.name}</h3>
                    </div>
                    <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/5">
                      <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest leading-none">Sector ID: {category.id}</span>
          ...
          <div className="bg-white/5 backdrop-blur-xl p-10 md:p-16 rounded-[3.5rem] border border-white/10 space-y-12 shadow-2xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-4">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] ml-2">Sector Identity</label>
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] ml-2">System Allocation Code</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-end">
              <div className="space-y-4">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] ml-2">Visual Descriptor (Emoji)</label>
                <div className="flex items-center space-x-6">
                  <input type="text" value={formData.icon} onChange={(e) => setFormData({ ...formData, icon: e.target.value })} className="flex-1 bg-black/40 border border-white/5 rounded-2xl px-8 py-5 text-white text-center text-xl" />
                  <div className="w-20 h-20 bg-white/5 rounded-3xl flex items-center justify-center text-4xl border border-white/10 shadow-2xl">{formData.icon}</div>
                </div>
              </div>
              <div className="space-y-4">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] ml-2">Logistics Priority Rank</label>
            ...
            <div className="p-8 bg-quali-primary/5 rounded-[2.5rem] border border-quali-primary/10 shadow-inner">
              <label className="flex items-center space-x-6 cursor-pointer group">
                ...
                <div>
                   <span className="text-sm font-black text-white uppercase tracking-widest italic block">Active System Node</span>
                   <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Enable sector in global logistics network</span>
                </div>
              </label>
            </div>

            <div className="pt-6 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
               <p className="text-[10px] font-black text-gray-600 uppercase tracking-[0.4em] italic">QualiFuel Admin Protocol v2.5</p>
               <div className="flex space-x-4">
                  ...
                  <button onClick={handleSaveCategory} className="bg-quali-gradient text-white px-12 py-5 rounded-2xl font-black uppercase tracking-widest text-xs shadow-2xl border border-white/10 hover:scale-[1.02] transform transition-all active:scale-95">Commit Changes</button>
               </div>
            </div>
          </div>
        </div>
